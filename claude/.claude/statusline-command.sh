...
# Human-readable label for the real context window size (e.g. 1M, 200k)
if (( size >= 1000000 )); then
  size_label=$(awk "BEGIN{printf \"%gM\", $size/1000000}")
else
  size_label="$(( size / 1000 ))k"
fi

# Personal soft limit (stay below this to avoid auto-compact / context bloat)
PERSONAL_LIMIT=120000
personal_pct=$(( current_tokens * 100 / PERSONAL_LIMIT ))
if (( current_tokens >= 1000 )); then
  tokens_fmt=$(awk "BEGIN{printf \"%.1fk\", $current_tokens/1000}")
else
  tokens_fmt="${current_tokens}"
fi
if (( current_tokens > PERSONAL_LIMIT )); then
  personal_icon="🚨"
else
  personal_icon="🎯"
fi
personal_color=$(color_pct "$personal_pct")
...
# Line 1: Model | Context % used | Tokens vs personal 120k | Dir (branch) | Duration
ctx_color=$(color_pct "$ctx_pct")
line1="${BLUE}${model}${RESET}${SEP}"
line1+="✍️ ${ctx_color}${ctx_pct}%${RESET} ${DIM}(${size_label})${RESET}${SEP}"
line1+="${personal_icon} ${personal_color}${tokens_fmt}${RESET}${DIM}/120k${RESET} ${personal_color}${personal_pct}%${RESET}${SEP}"
...
if [ -n "$usage" ]; then
  ...
  rate_lines="${WHITE}Current${RESET} ${fh_color}$(printf "%3d" "$fh_remaining")% left${RESET} ${DIM}⟳ ${RESET}${WHITE}${fh_reset}${RESET}"
  rate_lines+="\n${WHITE}Weekly${RESET}  ${wd_color}$(printf "%3d" "$wd_remaining")% left${RESET} ${DIM}⟳ ${RESET}${WHITE}${wd_reset}${RESET}"
